...
BOT_TOKEN = "8041853246:AAH4uA2aqnPGVHXHMz0hMDHXHdiIRchWwlQ"
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "⟡ /rain → 𝘽𝙍𝘼𝙄𝙉𝙏𝙍𝙀𝙀 𝘼𝙐𝙏𝙃 (𝙎𝙄𝙉𝙂𝙇𝙀)\n"
        "⟡ /mrain → 𝘽𝙍𝘼𝙄𝙉𝙏𝙍𝙀𝙀 𝘼𝙐𝙏𝙃 (𝙈𝘼𝙎𝙎)\n"
        "⟡ /stop ⟶ 𝙎𝙏𝙊𝙋 𝙈𝘼𝙎𝙎 𝘾𝙃𝙀𝘾𝙆𝙄𝙉𝙂",
        parse_mode="Markdown")
        
async def stop_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    task = context.user_data.get("mrain_task")
...
async def rain_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cc:
        await msg.reply_text("⟡ 𝗨𝗦𝗘 𝗟𝗜𝗞𝗘 ⟡  \n. ⫷ /rain cc|mm|yy|cvv", parse_mode="Markdown")
        return

    sent = await userbot.send_message(TARGET_BOT, f"/rain {cc}")
...
async def mrain_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message or not update.message.reply_to_message.document:
        return await update.message.reply_text("⟡ 𝗥𝗘𝗣𝗟𝗬 ⟡ /mrain 𝗜𝗡 𝗔 .txt 𝗙𝗜𝗟𝗘")
    ...
    context.user_data["mrain_task"] = task
...
async def init_all():
    ...
    app.add_handler(CommandHandler("rain", rain_handler))
    app.add_handler(CommandHandler("mrain", mrain_handler))

    await app.bot.set_my_commands([
        BotCommand("start", "𝙎𝙏𝘼𝙍𝙏 𝙏𝙃𝙀 𝘽𝗢𝙏"),
        BotCommand("stop", "𝙎𝙏𝙊𝙋 𝙈𝘼𝙎𝙎 𝘾𝙃𝙀𝘾𝙆𝙄𝙉𝙂"),
        BotCommand("rain", "𝘽𝙍𝘼𝙄𝙉𝙏𝙍𝙀𝙀 𝘼𝙐𝙏𝙃 (𝙎𝙄𝙉𝙂𝙇𝙀)"),
        BotCommand("mrain", "𝘽𝙍𝘼𝙄𝙉𝙏𝙍𝙀𝙀 𝘼𝙐𝙏𝙃 (𝙈𝘼𝙎𝙎)"),
    ])
